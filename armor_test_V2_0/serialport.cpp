 ...
 *
 * @param model 默认是1
 * @param model_select 模式选择:1为自瞄,2为大神符
 * @param x 点的x坐标
 * @param y 点的y坐标
 * @param depth 深度:自瞄模式默认给1,大神符模式默认给7200
 */
void SerialPort::sendData(int model,int model_select,int x,int y,int depth){
//    int model = 1;
//    int model_select = 1;
//    int depth = 1;
    sprintf(send_buf_temp, "%c%01d%01d%04d%03d%04d", 'S',model,model_select, x, y, depth);
    uint8_t CRC = Check_CRC8(send_buf_temp, sizeof(send_buf_temp));
    sprintf(send_buf, "%c%01d%01d%04d%03d%04d%03d%c", 'S',model, model_select,x, y,depth,CRC, 'E');
