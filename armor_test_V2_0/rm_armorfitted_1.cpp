...
//----------------------------------预处理-----------------------------------------
    cvtColor(copy_img,lab_img,COLOR_RGB2Lab);
    cvtColor(copy_img,gray_img,COLOR_RGB2GRAY);
#ifdef NUMBER
    Mat bin_img;
    Mat roi_number_img;
...
//--------------------------------------------双循环两两匹配灯条---------------------------------------
    ...
    RotatedRect last_armor_rect;
    RotatedRect candidate_rect;


    Point2f _pt[4],lu_i,ru_i,lu_j,ru_j;

    float light_addH_max = 0;
    float rect_area_max = 0;
...
#endif
//           cout<<width_i/light_distance<<endl;
//           if(width_i/light_distance)
//           light_rate_max = MAX(width_i/light_distance,light_rate_max);
//           if(width_i/light_distance)
//           {light_rate_min = MIN(width_i/light_distance,light_rate_min);}
//           cout<<"light_rate_max:"<<light_rate_max<<endl;
//           cout<<"light_rate_min:"<<light_rate_min<<endl;
//            bool condition1 = delta_w > min_light_delta_w && delta_w < max_light_delta_w;//状况一：判断两灯条的左右距离差差
//            bool  condition2= (delta_center_y < (rect_j.center.y+rect_i.center.y)*0.26 );//灯条的高度差 is_height_diff_catch
            bool  condition2= (delta_center_y < (leni+lenj)*0.5 );
//            cout<<"condition2:"<<condition2<<endl;
//            cout<<(leni+lenj)*0.5 <<endl;
            ...
            {
//                cout<<"成功进入"<<endl;
                candidate_rect = boundingRRect(rect_i, rect_j);//两灯条矩形的拟合矩形

                Point2f vertice[4];
                candidate_rect.points(vertice);
                for (int i = 0; i < 4; i++)
                    line(copy_img, vertice[i] + (Point2f)roi.tl, vertice[(i + 1) % 4] + (Point2f)roi.tl, Scalar(255, 255, 255), 2);
#ifdef DEDUG
...
//                cout<<"angle_right:"<<angle_right<<"      "<<"angle_left:"<<angle_left<<endl;
                bool is_light_angle_catch;//右正左负
                if( ( angle_left /angle_right > 0 )||( angle_right == 0 && angle_left == 0 ) ){
                    //同侧
...
//                cout<<"win:"<<is_light_angle_catch<<endl;//is_light_angle_catch 有问题
...
//                                cout<<"rect_j.size.height="<<rect_j.size.height<<endl;
                    ...
                    if( (leni + lenj >=light_addH_max) && (candidate_rect.size.width*candidate_rect.size.height >= rect_area_max))
                    {
                        is_last_data_catch = true;//检测到装甲板，则下一帧会标识上一帧有数据
                        is_Lost_target = false;//未丢失目标

                        last_armor_rect = boundingRRect(rect_i,rect_j);
                        //cout<<"last_armor_rect.size:"<<last_armor_rect.size<<endl;
                        light_addH_max = leni + lenj;
                        rect_area_max = candidate_rect.size.width*candidate_rect.size.height;

                        real_rect.rect = last_armor_rect;
//                        cout<<"light_addH_max:"<<light_addH_max<<endl;

                        Point2f vtx[4];
                        real_rect.rect.points(vtx);
                        for(int j=0;j<4;j++)
                        {
                            line(dst_img,vtx[j], vtx[(j + 1) % 4],Scalar(255,105,180),2,8,0);
                        }
                        circle(dst_img,real_rect.rect.center,2,Scalar(255,105,180),2,8,0);
...
//第二次循环结束
#endif
//    cout<<real_rect.rect.center<<endl;

    #if IS_KF_PREDICT_ARMOR_OPEN ==1
     Point2f kalman_point = kalman.point_Predict(g_runtime,real_rect.rect.center,is_last_data_catch);
     real_rect.rect.center.x=kalman_point.x;
     real_rect.rect.center.y=kalman_point.y;
//     cout<<real_rect.rect.center<<endl;

     circle(dst_img,kalman_point,2,Scalar(0,240,200),100,8,0);
    #endif
    #if SERIAL_IS_OPEN == 1
     if(is_last_data_catch)
     {
         int model = 1;
         int depth = 1;
         SerialPort::sendData(model,model_select,real_rect.rect.center.x,real_rect.rect.center.y,depth);k
     }
     else
     {
         int model = 0;
         int depth = 1;
         SerialPort::sendData(model,model_select,src_img.cols/2,src_img.rows/2,depth);
     }
    #endif
    cout<<"------------------------------------"<<endl;
